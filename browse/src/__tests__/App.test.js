...
it('renders and filters the restaurants', async () => {
  ...
  fetch.once(
    JSON.stringify([
      { id: '1', name: 'Fish and Chips', ...common },
      { id: '2', name: 'Chicken Rice', ...common },
      { id: '3', name: 'Fried Chicken', ...common },
    ]),
  );
  const app = mount(<App />);
  ...
  expect(app.find('BookCard')).toHaveLength(3);

  // There are 2 chicken restaurants
  app
    .find('input[type="text"]')
    .simulate('change', { target: { value: 'chicken' } });
  expect(app.find('BookCard')).toHaveLength(2);
  ...
  expect(app.find('BookCard')).toHaveLength(1);
});
